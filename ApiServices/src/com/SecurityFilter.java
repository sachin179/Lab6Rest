			...
			if (authHeader != null && authHeader.size() > 0) {
				...
				if (method.isAnnotationPresent(RolesAllowed.class)) {
					...
					if (rolesSet.contains("patient")) {
						webTarget = client.target("http://localhost:8081/AuthService/AuthService").path("users/buyer");

					} else if (rolesSet.contains("seller")) {
						webTarget = client.target("http://localhost:8081/AuthService/AuthService").path("users/seller");
